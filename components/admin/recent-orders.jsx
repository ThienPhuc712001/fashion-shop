...
export function RecentOrders({ orders = [] }) {
    return (<Card>
      ...
      <CardContent>
        {orders.length === 0 ? (<p className="text-center text-muted-foreground py-8">Chưa có đơn hàng nào</p>) : (<div className="space-y-4">
            {orders.slice(0, 5).map((order) => {
                var _a;
                const status = statusConfig[order.status] || statusConfig.pending;
                return (<div key={order.id} className="flex items-center justify-between gap-4 p-3 border rounded-lg">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground truncate">
                      {new Date(order.createdAt).toLocaleDateString('vi-VN')} • {((_a = order.items) === null || _a === void 0 ? void 0 : _a.length) || 0} sản phẩm
                    </p>
                  </div>
