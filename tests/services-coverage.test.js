...
describe('Service Coverage Boost', () => {
    ...
    describe('Payment Service', () => {
        ...
        it('verifyMomoPayment success', async () => {
            const result = await Payment.verifyMomoPayment({ resultCode: '0' });
            expect(result.success).toBe(true);
        });
        it('verifyMomoPayment failure', async () => {
            const result = await Payment.verifyMomoPayment({ resultCode: '100' });
        ...
        it('verifyVNPayPayment success', async () => {
            const result = await Payment.verifyVNPayPayment({ vnp_ResponseCode: '00' });
            expect(result.success).toBe(true);
        });
    });
    describe('Email Service', () => {
        ...
        it('sendPasswordReset', async () => {
            const result = await Email.sendPasswordReset({ email: 't@t.com' }, 'token');
            expect(result.to).toBe('t@t.com');
        });
        it('sendPromotionalEmail', async () => {
            const result = await Email.sendPromotionalEmail({ email: 't@t.com' }, 'Subj', 'Body');
