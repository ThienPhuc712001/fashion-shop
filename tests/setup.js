...
export const ensureDBConnected = async () => {
    ...
    try {
        ...
        console.warn('  Warning: createTables error (may be already created):', err.message);
    }
};
...
export const ensureTestData = async () => {
    ...
    if (!existingAdmin) {
       ...
       VALUES (?, ?, ?, ?, ?, ?)`, [uuidv4(), testUsers.admin.email, adminPasswordHash, testUsers.admin.name, 'admin', true]);
    ...
    if (!category) {
        categoryId = uuidv4();
        await db.run(`INSERT INTO categories (id, name, slug, is_active) VALUES (?, ?, ?, ?)`, [categoryId, 'Test Category', 'test-category', true]);
    ...
    if (!brand) {
        brandId = uuidv4();
        await db.run(`INSERT INTO brands (id, name, slug, is_active) VALUES (?, ?, ?, ?)`, [brandId, 'Test Brand', 'test-brand', true]);
    ...
    if (!product) {
        productId = uuidv4();
        await db.run(`INSERT INTO products (id, name, slug, description, category_id, brand_id, base_price, is_active) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [productId, 'Test Product', 'test-product', 'Test description', categoryId, brandId, 100000, true]);
        console.log('  ✅ Product created');
        // Create variant
        const variantId = uuidv4();
        await db.run(`INSERT INTO product_variants (id, product_id, sku, size, color_name, stock_quantity, is_available) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [variantId, productId, 'TEST-SKU-001', 'M', 'Black', 100, true]);
        console.log('  ✅ Variant created');
        // Create image
        await db.run(`INSERT INTO product_images (id, product_id, url, is_primary) 
       VALUES (?, ?, ?, ?)`, [uuidv4(), productId, 'https://via.placeholder.com/500', true]);
...
export const getTestIds = async () => {
    ...
    if (!product)
        throw new Error('Test product not found. Run ensureTestData first.');
...
export const registerTestUser = async (email, password, name) => {
