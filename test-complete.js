...
async function test(name, fn) {
    ...
    try {
        ...
        results.failed++;
        const error = ((_b = (_a = err.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.message) || err.message || 'Unknown error';
...
async function runTests() {
    ...
    await test('Upload Product Image', async () => {
        ...
        const res = await API.post(`/api/upload/products/${productId}/images`, form, {
            headers: Object.assign(Object.assign({}, form.getHeaders()), { Authorization: `Bearer ${adminToken}` })
        });
        if (!res.data.success)
            throw new Error('Failed');
    });
    ...
    // Write results
    fs.writeFileSync(path.join(process.cwd(), 'fashion-shop', 'complete-test-results.json'), JSON.stringify(results, null, 2));
