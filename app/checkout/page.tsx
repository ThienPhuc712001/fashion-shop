...
import { ErrorDisplay, Spinner, LoadingOverlay, LoadingPage } from '@/components/ui/spinner';
import { Address, Order, Coupon, Cart } from '@/lib/api';
...
export default function CheckoutPage() {
  ...
  const applyCoupon = async () => {
    ...
    setCouponError(null);
    try {
      const response = await api.post<Coupon>('/coupons/validate', { code: couponCode });
    ...
    try {
      const orderData: any = {
        ...
        shippingAddress: selectedAddressId
          ? addresses.find((a) => a.id === selectedAddressId) || { ...shippingInfo, isDefault: false }
          : { ...shippingInfo, isDefault: false },
        billingAddress: { ...shippingInfo, isDefault: false },
          ...
          {steps.map((step, index) => (
            <div key={step.id} className="flex items-center">
              ...
              {index < steps.length - 1 && (
                <div className="mx-4 h-0.5 w-12 sm:w-24 bg-muted" />
              )}
            </div>
          ))}
        </div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              {currentStep === 'payment' && (
                <div className="space-y-4">
                  ...
                  <label
                    className={`flex items-center gap-3 p-4 border rounded-lg cursor-pointer ${
                      paymentMethod === 'vnpay' ? 'border-primary bg-primary/5' : ''
                    }`}
                  >
                    <input
                      type="radio"
            ...
            <CardFooter className="flex justify-between">
              <Button variant="outline" onClick={() => {
                if (currentStep === 'shipping') router.push('/cart');
                else setCurrentStep((prev) => {
                  const idx = steps.findIndex((s) => s.id === prev);
                  return steps[idx - 1].id as CheckoutStep;
                });
              }}>
              ...
              {currentStep === 'review' ? (
                ...
                <Button
                  onClick={() => {
                    ...
                    setCurrentStep(steps[idx + 1].id as CheckoutStep);
                  }}
        ...
        <div className="lg:col-span-1">
          <Card className="sticky top-24">
            ...
            <CardContent className="space-y-4">
              ...
              {totals.shipping === 0 && (
                <p className="text-xs text-muted-foreground">Freeship cho đơn từ 500k</p>
              )}
              <Separator />
