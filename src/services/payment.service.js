...
export class MomoPaymentService {
    ...
    createPayment(orderId, amount, orderInfo) {
        ...
        const notifyUrl = encodeURIComponent(notifyUrl);
        const extraData = '';
        const rawSignature = `accessKey=${accessKey}&amount=${amountStr}&extraData=${extraData}&merchantId=${partnerCode}&orderId=${orderIdStr}&orderInfo=${orderInfoStr}&partnerCode=${partnerCode}&redirectUrl=${redirectUrl}&requestId=${requestId}&requestType=payment&version=2.0`;
        const signature = crypto.createHmac('sha256', secretKey).update(rawSignature).digest('hex');
        const paymentUrl = `https://payment.momo.vn/v2/gateway/api/create?partnerCode=${partnerCode}&accessKey=${accessKey}&requestId=${requestId}&amount=${amountStr}&orderId=${orderIdStr}&orderInfo=${orderInfoStr}&redirectUrl=${redirectUrl}&notifyUrl=${notifyUrl}&extraData=${extraData}&requestType=payment&signature=${signature}`;
        return {
            paymentUrl,
