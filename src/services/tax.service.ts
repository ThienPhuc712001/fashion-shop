interface TaxConfig {
  enabled: boolean;
  default_rate: number; // percentage, e.g., 10 for 10%
  rates_by_province: Record<string, number>; // province_code -> rate
}
...
class TaxService {
   ...
   * @returns Tax amount
   */
  calculateTax(subtotal: number, provinceCode?: string): number {
   ...
   */
  getBreakdown(subtotal: number, provinceCode?: string) {
    const rate = provinceCode && this.config.rates_by_province[provinceCode]
      ? this.config.rates_by_province[provinceCode]
      : this.config.default_rate;
