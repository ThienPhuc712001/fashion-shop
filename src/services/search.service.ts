...
class SearchService {
  ...
  async syncProduct(productId: string): Promise<void> {
    ...
    const existing = await db.get(
      'SELECT * FROM products_fts WHERE product_id = ?',
      [productId]
    );
    ...
    if (existing) {
      await db.run(
        `UPDATE products_fts SET 
         ...
         WHERE product_id = ?`,
        [product.name, product.description || '', product.category_name || '', product.brand_name || '', productId]
      );
    } else {
      await db.run(
        `INSERT INTO products_fts (product_id, name, description, category_name, brand_name)
         VALUES (?, ?, ?, ?, ?)`,
        [productId, product.name, product.description || '', product.category_name || '', product.brand_name || '']
      );
    }
  }
  ...
  async getSuggestions(prefix: string, limit: number = 10): Promise<string[]> {
    ...
    const suggestions = await db.all(
      `SELECT DISTINCT name as suggestion FROM products_fts 
       WHERE name LIKE ? 
       UNION
