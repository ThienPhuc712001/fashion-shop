...
const seed = async () => {
  try {
      ...
      if (!existing) {
        await db.run(
          `INSERT INTO categories (id, name, slug, description, sort_order, is_active) 
           VALUES (?, ?, ?, ?, ?, ?)`,
          [uuidv4(), cat.name, cat.slug, cat.description, cat.sort_order, true]
        );
    ...
    if (!clothingCat || !shoesCat) throw new Error('Failed to fetch category IDs');
    ...
    for (const brand of brandsData) {
      ...
      if (!existing) {
        await db.run(
          ...
          [uuidv4(), brand.name, brand.slug, true]
        );
    ...
    if (!nikeBrand || !adidasBrand) throw new Error('Failed to fetch brand IDs');
    ...
    for (const prod of productsData) {
      ...
      if (!existing) {
        productId = uuidv4();
        await db.run(
          `INSERT INTO products (id, name, slug, description, category_id, brand_id, base_price, is_active, created_at) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
          [productId, prod.name, prod.slug, prod.description, prod.category_id, prod.brand_id, prod.base_price, true]
        );
      ...
      for (const size of sizes) {
        for (const color of colors) {
          ...
          await db.run(
            `INSERT INTO product_variants (id, product_id, sku, size, color_name, color_hex, stock_quantity, is_available) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [variantId, productId, sku, size, color.name, color.hex, 50, true]
          );
        }
      }
      ...
      await db.run(
        `INSERT INTO product_images (id, product_id, url, alt_text, is_primary, sort_order) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [imageId, productId, imageUrl, prod.name, true, 0]
      );
    ...
    if (!existingAdmin) {
      ...
      await db.run(
        `INSERT INTO users (id, email, password_hash, full_name, role, email_verified) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [adminId, 'admin@fashionshop.com', adminPassword, 'Admin Fashion Shop', 'admin', true]
      );
