...
export const listCoupons = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const coupons = await db.all(
      `SELECT * FROM coupons ${whereClause} ORDER BY ${orderBy} LIMIT ? OFFSET ?`,
...
export const updateCoupon = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (max_discount_amount !== undefined) { updates.push('max_discount_amount = ?'); values.push(max_discount_amount); }
    if (usage_limit !== undefined) { updates.push('usage_limit = ?'); values.push(usage_limit); }
    if (per_user_limit !== undefined) { updates.push('per_user_limit = ?'); values.push(per_user_limit); }
...
export const applyCoupon = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const existing = await db.get(
      'SELECT * FROM order_coupons WHERE order_id = ? AND coupon_id = ?',
      [order_id, id]
    );
    if (existing) {
    ...
    await db.run(
      'UPDATE orders SET discount_amount = discount_amount + ? WHERE id = ?',
      [discount, order_id]
    );
