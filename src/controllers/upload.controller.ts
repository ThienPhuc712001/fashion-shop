...
export const uploadProductImage = [
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Save to database
      const sortOrder = await db.get(
        'SELECT MAX(sort_order) as max FROM product_images WHERE product_id = ?',
        [id]
      );

      await db.run(
        `INSERT INTO product_images (id, product_id, variant_id, url, alt_text, is_primary, sort_order) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [
          imageId,
          id,
...
export const listProductImages = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
