...
const generateMomoSignature = (params: any, secretKey: string): string => {
  const rawSignature = [
...
const generateVNPaySignature = (params: any, secretKey: string): string => {
...
export const createMomoPayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
  ...
  try {
    ...
    await db.run(
      `UPDATE orders 
       ...
       WHERE id = ?`,
      [orderStatus, data.transId, orderStatus === 'completed' ? 'paid' : 'pending', internalOrderId]
    );
...
export const createVNPayPayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const paymentUrl = `${vnpayUrl}/vnppay-payment/vnpay/payment.vnet?${querystring.stringify({
      ...params,
    ...
    await db.run(
      'UPDATE orders SET payment_gateway_txn_id = ?, status = ?, payment_status = ? WHERE id = ?',
      [vnp_TransactionNo, 'pending', 'pending', order_id]
    );
    ...
    await db.run(
      `UPDATE orders 
       SET payment_status = ?, 
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
      [status, transactionNo, isSuccess ? 'paid' : 'pending', order.id]
    );
