...
export const listProducts = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (category) { whereClause += ' AND p.category_id = ?'; params.push(category as string); }
    if (brand) { whereClause += ' AND p.brand_id = ?'; params.push(brand as string); }
    if (min_price) { whereClause += ' AND p.base_price >= ?'; params.push(Number(min_price)); }
    if (max_price) { whereClause += ' AND p.base_price <= ?'; params.push(Number(max_price)); }
    if (in_stock === 'true') {
    ...
    let sortCol = validSortColumns.includes(sort_by as string) ? sort_by as string : 'created_at';
...
export const createProduct = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await db.run(
      `INSERT INTO products (id, name, slug, description, category_id, brand_id, base_price, compare_price, is_active) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [productId, name, productSlug, description || null, category_id || null, brand_id || null, base_price, compare_price || null, is_active]
    );
...
export const createProductVariant = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await db.run(
      `INSERT INTO product_variants (id, product_id, sku, size, color_name, color_hex, price_adjustment, stock_quantity, is_available) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [variantId, productUuid, sku, size || null, colorName, color_hex || null, price_adjustment || 0, stock_quantity || 0, is_available !== false]
    );
...
export const updateProductVariant = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await db.run(
      `UPDATE product_variants SET ${updates.join(', ')} WHERE id = ? AND product_id = ?`,
      values
    );
