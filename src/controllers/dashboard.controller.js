...
export const getDashboardStats = async (req, res, next) => {
    try {
        ...
        const overview = await db.get(`SELECT 
       ...
       ORDER BY count DESC`);
        ...
        const topProducts = await db.all(`SELECT 
        p.id, p.name, p.slug,
       ...
       LIMIT 10`);
        // 4. Top categories
        const topCategories = await db.all(`SELECT 
        c.id, c.name, c.slug,
        COUNT(oi.id) as quantity_sold,
        SUM(oi.subtotal) as total_revenue
       FROM order_items oi
       ...
       LIMIT 10`);
        ...
        const salesTrend = await db.all(`SELECT 
       ...
       LIMIT 10`);
        // 8. Customer insights
        const customerStats = await db.get(`SELECT 
        COUNT(*) as total,
...
export const getRecentOrders = async (req, res, next) => {
    try {
        ...
        const orders = await db.all(`SELECT o.*, u.email as user_email, u.full_name as user_name
       FROM orders o
       LEFT JOIN users u ON o.user_id = u.id
       ${whereClause}
       ORDER BY o.created_at DESC
       LIMIT ? OFFSET ?`, [...params, Number(limit), Number(offset)]);
        const countResult = await db.get(`SELECT COUNT(*) as total FROM orders o ${whereClause}`, params);
        res.json({
            success: true,
            data: orders,
            pagination: {
...
export const getSalesReport = async (req, res, next) => {
    try {
       ...
       FROM orders o
       WHERE o.created_at BETWEEN ? AND ?`, [start_date, end_date]);
        res.json({
            success: true,
            data: {
...
export const getProductPerformance = async (req, res, next) => {
    try {
        ...
        const products = await db.all(`SELECT 
       ...
       LIMIT ?`, [Number(limit)]);
        ...
        const performance = products.map(p => (Object.assign(Object.assign({}, p), { conversion_rate: 0, avg_price: p.sold_quantity > 0 ? p.total_revenue / p.sold_quantity : 0 })));
        res.json({
            success: true,
            data: performance
        });
...
export const getInventoryStatus = async (req, res, next) => {
    try {
        const inventory = await db.all(`SELECT 
        p.id, p.name, p.slug,
       ...
       ORDER BY pv.stock_quantity ASC`);
        ...
        const summary = {
            ...
            low_stock_count: inventory.filter((i) => i.is_low_stock).length,
            out_of_stock_count: inventory.filter((i) => i.stock_quantity === 0).length,
            total_stock_value: inventory.reduce((sum, i) => sum + (i.stock_quantity * i.base_price || 0), 0)
        };
        res.json({
            success: true,
            data: {
