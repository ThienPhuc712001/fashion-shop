...
export const createOrder = async (req, res, next) => {
    ...
    try {
        ...
        const { shipping_address, billing_address, shipping_method, payment_method, coupon_code, notes, email } = req.body;
       ...
       WHERE ci.cart_session_id = ?`, [cartSession.id]);
        ...
        for (const item of cartItems) {
            if (item.quantity > item.stock_quantity) {
                throw new AppError(`Insufficient stock for ${item.product_name} (${item.size}, ${item.color_name}). Only ${item.stock_quantity} available.`, 400);
            }
        }
        ...
        if (coupon_code) {
         ...
         AND (expires_at IS NULL OR expires_at >= CURRENT_TIMESTAMP)`, [coupon_code]);
            if (!coupon) {
            ...
            if (coupon.min_order_amount && subtotal < coupon.min_order_amount) {
                throw new AppError(`Minimum order of ${coupon.min_order_amount} VND required to use this coupon`, 400);
            }
        ...
        const provinceCode = shipping_address === null || shipping_address === void 0 ? void 0 : shipping_address.province;
        ...
        if (applied_coupon_id && discount_amount > 0) {
            await db.run('INSERT INTO order_coupons (order_id, coupon_id, discount_applied) VALUES (?, ?, ?)', [orderId, applied_coupon_id, discount_amount]);
            await db.run('UPDATE coupons SET used_count = used_count + 1 WHERE id = ?', [applied_coupon_id]);
        }
...
export const getOrders = async (req, res, next) => {
    try {
        ...
        const orders = await db.all(`SELECT * FROM orders ${whereClause} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, Number(limit), offset]);
        res.json({
            success: true,
            data: {
                orders,
...
export const getOrder = async (req, res, next) => {
    ...
    try {
        ...
        const items = await db.all(`SELECT oi.*, pv.sku, pv.size, pv.color_name, pv.color_hex,
              p.name as product_name, p.slug as product_slug
       FROM order_items oi
       JOIN products p ON oi.product_id = p.id
       LEFT JOIN product_variants pv ON oi.variant_id = pv.id
       WHERE oi.order_id = ?`, [id]);
        const coupons = await db.all(`SELECT c.code, c.name, c.discount_type, c.discount_value, oc.discount_applied
