...
import { createPaymentService, PaymentConfig } from '../services/payment.service';
...
export const createPayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await db.run(
      `UPDATE orders SET payment_url = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND payment_status = 'pending'`,
      [paymentUrl, orderId]
    );
...
export const momoCallback = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await db.run(
      `UPDATE orders SET payment_status = ?, payment_transaction_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
      [status, transId, orderId]
    );
...
export const vnpayReturn = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const isValid = paymentService.vnpay.verifyCallback(params as any);

    if (!isValid) {
      return res.status(400).send('Invalid signature');
    }

    const { vnp_TxnRef, vnp_ResponseCode, vnp_TransactionStatus, vnp_Amount } = params as any;
...
export const vnpayIpn = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { vnp_TxnRef, vnp_TransactionStatus } = params as any;
    const orderId = vnp_TxnRef;
    const status = vnp_TransactionStatus === '00' ? 'paid' : 'failed';
