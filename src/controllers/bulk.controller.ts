...
export const exportProductsCSV = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (brand_id) {
      sql += ' AND p.brand_id = ?';
      params.push(brand_id as string);
    }
...
export const importProductsCSV = async (req: Request, res: Response, next: NextFunction) => {
  try {
    res.json({
      success: true,
...
export const exportVariantsCSV = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const bulkUpdateVariants = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    for (const update of updates) {
      const result = await db.run(
        updatesSql,
        [update.stock_quantity, update.price_adjustment, update.variant_id, product_id]
      );
