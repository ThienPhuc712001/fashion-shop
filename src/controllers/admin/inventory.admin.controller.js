...
export const getAdminInventory = async (req, res, next) => {
    try {
        const { page = 1, limit = 50, lowStock, search } = req.query;
    ...
    `;
      ...
      SELECT 
        ...
        v.stock_quantity,
        v.reserved_quantity,
        (v.stock_quantity - v.reserved_quantity) as available,
    ...
    `;
        ...
        if (lowStock && String(lowStock).toLowerCase() === 'true') {
            countSql += ' AND (v.stock_quantity - v.reserved_quantity) <= v.low_stock_threshold';
            dataSql += ' AND (v.stock_quantity - v.reserved_quantity) <= v.low_stock_threshold';
        }
        if (search) {
        ...
        dataSql += ' ORDER BY p.name, v.sort_order LIMIT ? OFFSET ?';
      ...
      SELECT 
        ...
        SUM(CASE WHEN (v.stock_quantity - v.reserved_quantity) <= v.low_stock_threshold THEN 1 ELSE 0 END) as low_stock_count,
...
export const updateVariantStock = async (req, res, next) => {
    try {
        ...
        const variant = await db.get('SELECT * FROM product_variants WHERE id = ?', [variant_id]);
        if (!variant) {
...
export const bulkUpdateVariantStock = async (req, res, next) => {
    try {
        ...
        for (const update of updates) {
            ...
            await db.run(`UPDATE product_variants SET stock_quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [stock_quantity, variant_id]);
        }
