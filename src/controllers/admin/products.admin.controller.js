...
export const getAdminProduct = async (req, res, next) => {
    try {
        const { id } = req.params;
        const product = await db.get(`SELECT p.*, c.name as category_name, c.slug as category_slug
       FROM products p
...
export const createProduct = async (req, res, next) => {
    try {
        const { name, description, base_price, cost, category_id, status = 'active', slug, image_url } = req.body;
        const productSlug = slug || generateSlug(name);
        const productId = `prod_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        await db.run(`INSERT INTO products (
        id, name, slug, description, base_price, cost, 
        category_id, status, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, [productId, name, productSlug, description || null, base_price || 0, cost || 0, category_id, status]);
        ...
        if (image_url) {
         ...
         VALUES (?, ?, ?, ?, ?, ?)`, [`img_${Date.now()}`, productId, image_url, name, 1, 0]);
        }
        res.status(201).json({
            success: true,
...
export const updateProduct = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { name, description, base_price, cost, category_id, status, slug, image_url } = req.body;
        // Check exists
        ...
        cost = ?, category_id = ?, status = ?, updated_at = CURRENT_TIMESTAMP 
       WHERE id = ?`, [name, productSlug, description || null, base_price || 0, cost || 0, category_id, status, id]);
        ...
        if (image_url) {
            ...
            await db.run(`INSERT INTO product_images (id, product_id, url, alt_text, is_primary, sort_order) 
         VALUES (?, ?, ?, ?, ?, ?)`, [`img_${Date.now()}`, id, image_url, name, 1, 0]);
        }
        res.json({
            success: true,
...
export const getProductVariants = async (req, res, next) => {
    try {
        ...
        const variants = await db.all(`SELECT * FROM product_variants WHERE product_id = ? ORDER BY sort_order`, [product_id]);
        res.json({
            success: true,
            data: variants
        });
...
export const updateVariantStock = async (req, res, next) => {
    try {
        ...
        await db.run(`UPDATE product_variants SET 
        stock_quantity = ?, reserved_quantity = ?, updated_at = CURRENT_TIMESTAMP 
       WHERE id = ?`, [stock_quantity, reserved_quantity || 0, variant_id]);
       ...
       WHERE id = (SELECT product_id FROM product_variants WHERE id = ?)`, [variant_id, variant_id]);
        res.json({
            success: true,
