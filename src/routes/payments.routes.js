...
import { createMomoPayment, handleMomoIPN, createVNPayPayment, handleVNPayIPN } from '../controllers/payments.controller';
import { body } from 'express-validator';
import { authenticate } from '../middleware/auth';
import { paymentLimiter } from '../middleware/rateLimit';
// Momo
router.post('/momo/create', authenticate, paymentLimiter, body('order_id').isString(), body('amount').isFloat({ min: 1 }), body('order_info').optional().isString(), createMomoPayment);
...
// VNPay
router.post('/vnpay/create', authenticate, paymentLimiter, body('order_id').isString(), body('amount').isFloat({ min: 1 }), createVNPayPayment);
