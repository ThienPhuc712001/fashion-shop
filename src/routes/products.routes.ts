...
router.put('/:id', authenticate, authorize('admin', 'seller'), [
  param('id').isString(),
  ...createProductValidation.filter(r => Array.isArray(r) && r.length > 0 && (r[0] as any).exists === false)
], updateProduct);
...
router.put('/:id/variants/:variantId', authenticate, authorize('admin', 'seller'), [
  ...
  ...variantValidation.filter(r => Array.isArray(r) && r.length > 0 && (r[0] as any).optional === true)
], updateProductVariant);
